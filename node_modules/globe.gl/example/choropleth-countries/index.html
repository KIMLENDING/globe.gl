...
<body>
  ...
  <label for="countryArray">종에 따른 나라:</label>
  <button id="highlightMatchingCountries">표시</button>
  ...
  <script>
    ...
    fetch('../datasets/ne_110m_admin_0_countries.geojson').then(res => res.json()).then(countries => {
      ...
      const world = Globe()
        ...
        .polygonCapColor(() => 'rgba(255, 255, 200,1)')
    ...
    `)
        .onPolygonHover(hoverD => world
          ...
          .polygonCapColor(d => d === hoverD ? 'green' : 'rgb(255, 255, 200)')
        )
        .polygonsTransitionDuration(300)
        (document.getElementById('globeViz'))
      // 사용자 입력을 받을 input 요소 추가
      const inputElement = document.getElementById('countryInput');
      const buttonElement = document.getElementById('highlightMatchingCountries');
      ...
      function highlightCountryByName(countryName) {
        ...
        if (matchingCountry) {
          ...
          world.polygonCapColor(d => d === matchingCountry ? 'red' : 'rgb(255, 255, 200)');
        } else {
          // 국가를 찾지 못한 경우 기본 설정으로 돌아감
          world.polygonAltitude(0.06);
          world.polygonCapColor(() => 'rgb(255, 255, 200)');
        }
      }
      
      
      // 사용자가 입력한 국가명에 해당하는 다각형을 찾아 높이를 조절하는 함수
      function highlightMatchingCountries() {
        const matchingCountries = countries.features.filter(d => countriess.includes(d.properties.ADMIN));
        //highlightCountries(matchingCountries); // 입력값에 해당하는 나라들을 강조
        console.log(matchingCountries);
        highlightCountries(matchingCountries);
      }

      // 동시에 여러 다각형을 강조하는 함수
      function highlightCountries(countryList) {
        // 모든 나라들의 높이와 색상을 한 번에 설정
        world.polygonAltitude(d => countryList.includes(d) ? 0.12 : 0.06);
        world.polygonCapColor(d => countryList.includes(d) ? 'red' : 'rgb(255, 255, 200)');
      }

      buttonElement.addEventListener('click', highlightMatchingCountries); // 버튼을 클릭하면 highlightMatchingCountries 함수 호출
    ...
    async function fetchRedListData() {
      try {
        ...
        if (response_countries.ok) {
          ...
          countriess = extractCountryNames(data);
          displayResultCountries(countriess);
        } else {
    ...
    // extractCountryNames 함수의 반환값인 국가 이름 배열을 받아 국가 코드 배열로 변환하는 함수
    function convertCountryNamesToCodes(countryNames) {
      ...
      countryNames.forEach(countryName => {
        ...
        if (matchingCountry) {
          countryCodes.push(matchingCountry.properties.ISO_A2);
        }
      });
